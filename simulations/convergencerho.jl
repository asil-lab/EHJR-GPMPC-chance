...
# Save the simulation results
    ...
    return (
        ...
        mse=mse_histories,
        paths=path_histories,
        primal_residuals=convergence_history["primal_residuals"],
        iterations=convergence_history["iterations"]  # Add this line
    ), ref_trajs
end
...
Random.seed!(1235)  # For reproducibility
# results, ref_trajs = compare_dynamics_modes()

println("\nComparison complete! Check the generated plots and saved data.")

function compare_rho_values()
    # Basic setup
    num_agents = 3
    num_steps = 50
    max_steps = 150
    horizon = 13
    rho_values = [2.0, 20.0, 200.0]
    
    # Define dynamics modes
    modes = Dict(
        "Nominal-NoChance" => (Nominal, false),
        "Nominal-Chance" => (Nominal, true),
        "Nonlinear-NoChance" => (Nonlinear, false),
    ...
    # Generate reference trajectories (same for all tests)
    ref_trajs = generate_crossing_trajectories(num_steps, num_agents, radius=4.0, spacing=1.0)
    ...
    # Test each mode with different rho values
    for (mode_name, (dynamics_mode, use_chance)) in modes
        ...
        for rho in rho_values
            ...
            # Copy trained GP models and initial states
            for (agent, trained_agent, ref_traj) in zip(agents, trained_agents, ref_trajs)
                agent.gp_model = trained_agent.gp_model
                reset_state!(agent, ref_traj[:, 1])
            end
            
            results, _ = run_distributed_comparison(controller, agents, ref_trajs, 
                                                 max_steps=max_steps, goal_threshold=0.1)
            
            mode_results[rho] = results
        end
        
        convergence_results[mode_name] = mode_results
    end
    
    # Save all results
    save("rho_comparison_$timestamp.jld2", Dict(
    ...
    # Plot convergence for each mode
    plot_rho_comparison(convergence_results, timestamp)
    
    return convergence_results
end

function plot_rho_comparison(convergence_results, timestamp=Dates.format(now(), "yyyy-mm-dd_HH-MM-SS"))
    for (mode_name, rho_results) in convergence_results
        p = plot(
            title="ADMM Convergence - $mode_name",
            xlabel="ADMM Iteration",
            ylabel="Mean Primal Residual (log scale)",
            yscale=:log10,
            legend=:topright,
            grid=false
        )
        
        for (rho, results) in rho_results
            all_timestep_residuals = results.primal_residuals
            max_iter = maximum(length.(all_timestep_residuals))
            mean_residuals = zeros(max_iter)
            
            # For each iteration, find mean across all timesteps
            for iter in 1:max_iter
                iter_residuals = Float64[]
                for timestep_residuals in all_timestep_residuals
                    if iter <= length(timestep_residuals)
                        # Use actual value if available
                        push!(iter_residuals, timestep_residuals[iter])
                    else
                        # Use convergence tolerance if iteration wasn't needed
                        push!(iter_residuals, 1e-9)
                    end
                end
                mean_residuals[iter] = mean(iter_residuals)
            end
            
            plot!(p, mean_residuals,
                label="ρ = $rho",
                linewidth=2,
                marker=:circle,
                markersize=3,
        ...
        savefig(p, "convergence_$(mode_name)_$timestamp.png")
        display(p)
    end
end

# Update the load_and_plot_rho_comparison function
function load_and_plot_rho_comparison(filename)
    data = load(filename)
...
# Run the comparison
# println("Starting rho comparison...")
# Random.seed!(1235)
# results = compare_rho_values()
# println("\nComparison complete! Check the generated plots.")

# To load and re-plot from a saved file, use:
loaded_results = load_and_plot_rho_comparison("rho_comparison_2025-08-25_09-55-41.jld2")